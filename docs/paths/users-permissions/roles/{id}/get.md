...
<Url>
  <Method color="#6b55b2">GET</Method><Path>{unescape(escape('/users-permissions/roles/{id}'))}</Path>
</Url>
...
```shell
curl --request GET \
  --url https://maiascore.com/users-permissions/roles/%7Bid%7D \
...
const options = {
  ...
  port: null,
  path: "/users-permissions/roles/%7Bid%7D",
  headers: {
...
conn.request("GET", "/users-permissions/roles/%7Bid%7D", headers=headers)
...
curl_setopt_array($curl, [
  CURLOPT_URL => "https://maiascore.com/users-permissions/roles/%7Bid%7D",
